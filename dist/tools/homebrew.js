...
// ─── Monster Form ────────────────────────────────────────────────────────────
async function fillMonsterForm(page, data) {
    await page.evaluate(({ data, SIZE_MAP, ALIGNMENT_MAP, CR_MAP }) => {
        ...
        const setDescription = (baseName, html) => {
            // Set markup type to Raw Html (value "6")
            const typeField = document.getElementById(`field-${baseName}-type`);
            if (typeField) {
                typeField.value = "6";
            ...
            if (wysiwygEl) {
                wysiwygEl.value = html;
            }
            // Set TinyMCE editor content
            if (typeof window.tinymce !== "undefined") {
                const editor = window.tinymce.get(`field-${baseName}-wysiwyg`);
                if (editor)
                    editor.setContent(html);
            }
        };
        const toHtml = (entries) => entries.map(e => e.name === "_intro"
            ? `<p>${e.description}</p>`
            : `<p><em><strong>${e.name}.</strong></em> ${e.description}</p>`).join("\n");
        // Basic fields
        if (data.name !== undefined)
            setInput("field-Name", data.name);
        if (data.size !== undefined)
            setSelect("field-size", SIZE_MAP[data.size] ?? "");
        if (data.alignment !== undefined)
            setSelect("field-alignment", ALIGNMENT_MAP[data.alignment] ?? "");
        if (data.ac !== undefined)
            setInput("field-armor-class", data.ac);
        if (data.ac_type !== undefined)
            setInput("field-armor-class-type", data.ac_type);
        if (data.hp !== undefined)
            setInput("field-average-hit-points", data.hp);
        if (data.passive_perception !== undefined)
            setInput("field-passive-perception", data.passive_perception);
        // Challenge Rating
        if (data.challenge !== undefined) {
            const crNum = data.challenge.match(/^([\d/]+)/)?.[1] ?? data.challenge;
            const crValue = CR_MAP[crNum];
            if (crValue)
                setSelect("field-challenge-rating", crValue);
        }
        // Hit Dice
        ...
        if (data.type !== undefined) {
            const typeSelect = document.getElementById("field-monster-type");
            if (typeSelect) {
                const option = Array.from(typeSelect.options).find(o => o.text.toLowerCase() === data.type.toLowerCase());
                if (option) {
        ...
        if (data.traits !== undefined)
            setDescription("special-traits-description", toHtml(data.traits));
        if (data.actions !== undefined)
            setDescription("actions-description", toHtml(data.actions));
        if (data.bonus_actions !== undefined)
            setDescription("bonus-actions-description", toHtml(data.bonus_actions));
        if (data.reactions !== undefined)
            setDescription("reactions-description", toHtml(data.reactions));
        if (data.legendary_actions !== undefined)
            setDescription("legendary-actions-description", toHtml(data.legendary_actions));
        if (data.mythic_actions !== undefined)
            setDescription("mythic-actions-description", toHtml(data.mythic_actions));
        if (data.lair !== undefined)
            setDescription("lair-description", toHtml(data.lair));
        if (data.lore !== undefined)
            setDescription("monster-characteristics-description", `<p>${data.lore}</p>`);
    }, {
        data: data,
        SIZE_MAP,
        ALIGNMENT_MAP,
...
// ─── Monster CRUD ────────────────────────────────────────────────────────────
export async function editMonster(context, id, data) {
...
export async function createMonster(context, data) {
    if (!data.name)
        throw new Error("Monster name is required for creation.");
    const page = await getPage(context);
...
async function fillItemForm(page, data) {
    await page.evaluate(({ data, ITEM_RARITY_MAP }) => {
        ...
        if (data.rarity !== undefined) {
            ...
            if (rarityValue)
                setSelect("field-rarity", rarityValue);
        }
        // Item type — field-type select, match by option text
        if (data.type !== undefined) {
            const typeSelect = document.getElementById("field-type");
            if (typeSelect) {
                const option = Array.from(typeSelect.options).find(o => o.text.toLowerCase().includes(data.type.toLowerCase()));
                if (option) {
                    typeSelect.value = option.value;
                    typeSelect.dispatchEvent(new Event("change", { bubbles: true }));
                }
        ...
        if (data.attunement !== undefined) {
            const el = document.getElementById("field-requires-attunement");
        ...
        if (data.description !== undefined) {
            ...
            const typeField = document.getElementById("field-item-description-type");
            if (typeField) {
            ...
            if (wysiwygEl) {
                wysiwygEl.value = data.description;
            }
            // Set TinyMCE editor
            if (typeof window.tinymce !== "undefined") {
                const editor = window.tinymce.get("field-item-description-wysiwyg");
                if (editor)
                    editor.setContent(data.description);
            }
        }
    }, { data: data, ITEM_RARITY_MAP });
}
// ─── Item CRUD ───────────────────────────────────────────────────────────────
function getItemFormId(page) {
    return page.evaluate(() => {
        const form = document.querySelector("#magic-item-form, form[id*='item'], form[id*='magic']");
        return form?.id ?? null;
    });
}
export async function editItem(context, id, data) {
    const page = await getPage(context);
    ...
    if (!formId)
        throw new Error(`Item form not found at ${editUrl}. Check the item ID.`);
...
export async function createItem(context, data) {
    if (!data.name)
        throw new Error("Item name is required for creation.");
    const page = await getPage(context);
...
// ─── Delete ──────────────────────────────────────────────────────────────────
export async function deleteHomebrew(context, type, id) {
    const page = await getPage(context);
    ...
    await page.evaluate(() => {
        ...
        const confirmBtn = document.querySelector(".vex-dialog-button-primary, .vex-dialog-buttons button:first-child");
        if (confirmBtn) {
            confirmBtn.click();
            return;
        }
        ...
        const okBtn = buttons.find(b => /^(ok|yes|confirm|delete)$/i.test(b.textContent?.trim() ?? ""));
        if (okBtn)
            okBtn.click();
    });
    await page.waitForTimeout(5000);

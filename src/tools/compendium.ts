 ...
 */
const MY_CREATIONS_TYPE_MAP: Record<string, string[]> = {
  monsters: ["monster"],
  spells: ["spell"],
  "magic-items": ["item"],
  feats: ["feat"],
...
async function findLinkByText(
  ...
  return page.evaluate(
    ({ q, segments }: { q: string; segments?: string[] }) => {
      ...
      for (const link of links) {
        ...
        if (href.includes("/search") || href.includes("/auth") || href.includes("#")) continue;
        // If we have URL segments to match, require them
        if (segments && !segments.some((seg) => href.includes(seg))) continue;
        // Match by name
 ...
 */
async function searchAndGetFirstUrl(
  context: BrowserContext,
  category: string,
  query: string,
  source: CompendiumSource = "official"
): Promise<{ page: Page; url: string; name: string }> {
  ...
  if (source === "my-creations") {
    ...
    const matchedName = await page.evaluate(
      ({ q }: { q: string }) => {
        const normalizedQuery = q.toLowerCase().trim();
        ...
        for (const link of links) {
          ...
          if (name.toLowerCase().includes(normalizedQuery) || normalizedQuery.includes(name.toLowerCase())) {
            ...
            if (/^(next|previous|1|2|3|\d+)$/i.test(name)) continue;
            return name;
          }
        }
        return null;
  ...
  if (source === "homebrew") {
    ...
    const searchInput = await page.$(
      'input[placeholder*="Name"], input[name*="filter-search"], input[name*="name"]'
    );
    if (searchInput) {
  ...
  let firstResult = await page.evaluate(() => {
    const listingLink = document.querySelector(".listing-body div.info[data-slug] a.link") as HTMLAnchorElement | null;
    if (listingLink) {
      return { name: listingLink.textContent?.trim() ?? "", url: listingLink.href };
    }
    const rowLink = document.querySelector(".listing-body .row a[href], .rpgdata-listing a.name-link, .results a.link") as HTMLAnchorElement | null;
    if (rowLink) {
  ...
  if (!firstResult || !firstResult.url) {
    ...
    firstResult = await page.evaluate((urlSegments: string[]) => {
      const links = Array.from(document.querySelectorAll("a[href]")) as HTMLAnchorElement[];
      for (const link of links) {
        const href = link.href;
 ...
 */
export async function listMyCreations(
  context: BrowserContext,
  type?: string
): Promise<string> {
  const page = await getPage(context);
  ...
  const scrapeCurrentPage = async () => {
    return page.evaluate((getCleanTextStr: string) => {
      const getCleanText = new Function(`return (${getCleanTextStr})`)() as () => string;
      const text = getCleanText();
      const lines = text.split("\n").map((l) => l.trim()).filter(Boolean);
      const items: Array<{ name: string; type: string; status: string }> = [];
  ...
  const creations: Array<{ name: string; type: string; status: string }> = [];
...
const GET_CLEAN_TEXT = () => {
  ...
  let text = (main as HTMLElement).innerText ?? "";
  // Cut off at comments section
  const commentsIdx = text.indexOf("\nComments\n");
  if (commentsIdx > -1) text = text.substring(0, commentsIdx);
  return text.trim();
};

// ─── getMonster ──────────────────────────────────────────────────────────────

export async function getMonster(context: BrowserContext, query: string, source: CompendiumSource = "official"): Promise<string> {
  const { page, url } = await searchAndGetFirstUrl(context, "monsters", query, source);

  const monster = await page.evaluate((getCleanTextStr: string) => {
    const getCleanText = new Function(`return (${getCleanTextStr})`)() as () => string;
    const text = getCleanText();
    const lines = text.split("\n").map((l) => l.trim()).filter(Boolean);
    ...
    const abilities: Record<string, { score: number; modifier: string; save: string }> = {};
    ...
    const tidbitPatterns: Record<string, RegExp> = {
      skills: /Skills\s+(.+)/,
      senses: /Senses\s+(.+)/,
    ...
    const sections: Record<string, ActionEntry[]> = {};
    let currentSection = "traits";
    sections[currentSection] = [];

    const entryLines = afterCRLines.split("\n");
    let currentEntry: ActionEntry | null = null;
    let loreText = "";
    ...
    for (const line of entryLines) {
      ...
      if (!trimmed) continue;
      if (chromeLines.test(trimmed)) continue;
      ...
      if (isActionEntry) {
        ...
        const hasMoreSections = entryLines.slice(currentIdx + 1).some((l) =>
          sectionHeaders.some((h) => h.toLowerCase() === l.trim().toLowerCase())
        );
        ...
        if (currentEntry) {
          ...
          if (remaining) loreText = remaining;
          break;
        }
      }
    }
...
export async function getSpell(context: BrowserContext, query: string, source: CompendiumSource = "official"): Promise<string> {
  const { page, url } = await searchAndGetFirstUrl(context, "spells", query, source);

  const spell = await page.evaluate((getCleanTextStr: string) => {
    const getCleanText = new Function(`return (${getCleanTextStr})`)() as () => string;
    const text = getCleanText();
    ...
    const higherLevelIdx = descriptionLines.findIndex((l) =>
      /^using a higher[- ]level spell slot/i.test(l) || /^at higher levels/i.test(l)
    );
...
export async function getItem(context: BrowserContext, query: string, source: CompendiumSource = "official"): Promise<string> {
  const { page, url } = await searchAndGetFirstUrl(context, "magic-items", query, source);

  const item = await page.evaluate((getCleanTextStr: string) => {
    const getCleanText = new Function(`return (${getCleanTextStr})`)() as () => string;
    const text = getCleanText();
    const lines = text.split("\n").map((l) => l.trim()).filter(Boolean);
    ...
    while (startIdx < lines.length) {
      ...
      if (rarityKeywords.test(line) || itemTypeKeywords.test(line)) break;
      startIdx++;
    }
...
export async function getFeat(context: BrowserContext, query: string, source: CompendiumSource = "official"): Promise<string> {
  const { page, url } = await searchAndGetFirstUrl(context, "feats", query, source);

  const feat = await page.evaluate((getCleanTextStr: string) => {
    const getCleanText = new Function(`return (${getCleanTextStr})`)() as () => string;
    const text = getCleanText();
    const lines = text.split("\n").map((l) => l.trim()).filter(Boolean);
...
export async function getRace(context: BrowserContext, query: string, source: CompendiumSource = "official"): Promise<string> {
  const { page, url } = await searchAndGetFirstUrl(context, "races", query, source);

  const race = await page.evaluate((getCleanTextStr: string) => {
    const getCleanText = new Function(`return (${getCleanTextStr})`)() as () => string;
    const text = getCleanText();
...
export async function getClass(context: BrowserContext, query: string, source: CompendiumSource = "official"): Promise<string> {
  const { page, url } = await searchAndGetFirstUrl(context, "classes", query, source);

  const classData = await page.evaluate((getCleanTextStr: string) => {
    const getCleanText = new Function(`return (${getCleanTextStr})`)() as () => string;
    const text = getCleanText();
...
export async function getBackground(context: BrowserContext, query: string, source: CompendiumSource = "official"): Promise<string> {
  const { page, url } = await searchAndGetFirstUrl(context, "backgrounds", query, source);

  const background = await page.evaluate((getCleanTextStr: string) => {
    const getCleanText = new Function(`return (${getCleanTextStr})`)() as () => string;
    const text = getCleanText();
...
export async function getCondition(context: BrowserContext, query: string): Promise<string> {
  const page = await getPage(context);
  ...
  const condition = await page.evaluate((searchQuery: string) => {
    ...
    if (!matchedHeading) {
      ...
      const capitalizedQuery = searchQuery.trim().split(/\s+/).map(
...
export async function getRule(context: BrowserContext, query: string): Promise<string> {
  const page = await getPage(context);

  // Helper to extract a rule from a page by heading match
  async function extractRuleFromPage(pageUrl: string): Promise<{ name: string; description: string } | null> {
    await page.goto(pageUrl, { waitUntil: "networkidle", timeout: 30000 });
    await page.waitForTimeout(1000);

    return page.evaluate((searchQuery: string) => {
      const normalizedQuery = searchQuery.toLowerCase().trim();
      const headings = Array.from(document.querySelectorAll("h2, h3, h4, h5"));
      let matchedHeading: Element | null = null;
      let bestScore = 0;
      ...
      if (!matchedHeading || bestScore < 10) return null;
...
export async function getEquipment(context: BrowserContext, query: string, source: CompendiumSource = "official"): Promise<string> {
  const { page, url } = await searchAndGetFirstUrl(context, "equipment", query, source);

  const equipment = await page.evaluate((getCleanTextStr: string) => {
    const getCleanText = new Function(`return (${getCleanTextStr})`)() as () => string;
    const text = getCleanText();
